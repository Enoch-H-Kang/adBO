...
python run_gepa_hover_compare.py \
  --out_root "logs/run1" \
  --refresh_sec 20 \
  --stage_step 500 \
  --seed 0 \
...
def plot_curves_png(out_png: Path, curves: dict[str, tuple[list[int], list[float]]], title: str):
    ...
    plt.ylabel("Best validation recall so far")
    plt.title(title)
...
def main():
    ...
    ap.add_argument("--worker", type=str, default="run_gepa_hover_worker.py",
    ...
    while True:
        ...
        if curves:
            plot_curves_png(
                live_png,
                curves,
                title="HoVer: GEPA variant comparison (live)"
            )
    ...
    if curves:
        plot_curves_png(final_png, curves, title="HoVer: GEPA variant comparison (final)")
        _, rows = merge_step_curves(curves)
        write_merged_csv(merged_csv, list(curves.keys()), rows)
